"""FaderBank — N faders each bound to a Parameter via a ParameterProvider.
Faders send absolute CC values (not deltas). Default column_offset=4 places
labels in the right half of the SL MkIII screens.
"""
...
class FaderBank(Widget):
    def __init__(self,
                 provider: ParameterProvider,
                 first_slot: int = 0,
                 *,
                 label_color: int = int(Color.DIM_WHITE),
                 column_offset: int = 4):
        n = provider.size()
        self.region = WidgetRegion(fader_slots=range(first_slot, first_slot + n))
